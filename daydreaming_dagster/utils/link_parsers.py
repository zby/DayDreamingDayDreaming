...
# Registry of available link-phase parsers
# Keyed by parser name as referenced in data/1_raw/link_templates.csv (column `parser`).
LINK_PARSERS_REGISTRY: Dict[str, Callable[[str], str]] = {
    "essay_idea_last": parse_essay_idea_last,
}


def get_link_parser(name: str) -> Optional[Callable[[str], str]]:
