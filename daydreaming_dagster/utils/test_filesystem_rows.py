...
from daydreaming_dagster.utils.filesystem_rows import get_row_by_gen_id, read_metadata


def test_read_metadata_present(tmp_path: Path):
    # Arrange: create gens_root/stage/gen_id/ with metadata.json
    docs_root = tmp_path / "gens"
    stage = "draft"
    gen_id = "abc123"
    base = docs_root / stage / gen_id
    base.mkdir(parents=True)
    meta = {"task_id": "t1", "template_id": "tplX", "model_id": "mA"}
    (base / "metadata.json").write_text(json.dumps(meta), encoding="utf-8")

    # Act
    row = get_row_by_gen_id(docs_root, stage, gen_id)
