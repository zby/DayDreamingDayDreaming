    ...
    *,
    ...
    if df.empty:
        raise Failure(
            ...
            metadata={
                "function": MetadataValue.text("resolve_essay_generator_mode"),
                "data_root": MetadataValue.path(str(data_root)),
                "resolution": MetadataValue.text(
                    "Ensure data/1_raw/essay_templates.csv contains templates with a 'generator' column"
                ),
            },
        )
    ...
    if row.empty:
        raise Failure(
            ...
            metadata={
                ...
                "resolution": MetadataValue.text(
                    "Add the template to essay_templates.csv or correct the essay template_id in tasks"
                ),
            },
        )
    ...
    if mode not in ("llm", "copy"):
        raise Failure(
            ...
            metadata={
                "function": MetadataValue.text("resolve_essay_generator_mode"),
                "template_id": MetadataValue.text(str(template_id)),
                "resolution": MetadataValue.text("Set generator to 'llm' or 'copy' in essay_templates.csv"),
            },
        )
