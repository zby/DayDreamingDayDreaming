#!/usr/bin/env python3
"""
Scan selected combo mappings for concept_ids missing from concepts_metadata.csv.

Usage:
  uv run python scripts/find_missing_selected_concepts.py \
    --data-root data \
    --selected data/2_tasks/selected_combo_mappings.csv \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--selected", type=Path, default=Path("data/2_tasks/selected_combo_mappings.csv"), help="Path to selected_combo_mappings.csv")
...
def load_selected_concepts(path: Path) -> pd.DataFrame:
    try:
        df = pd.read_csv(path)
    except Exception as e:
        print(f"ERROR: failed to read selected_combo_mappings from {path}: {e}", file=sys.stderr)
        raise SystemExit(1)
...
def main() -> int:
    args = parse_args()
    selected_df = load_selected_concepts(args.selected)
    concepts_df = load_concepts_metadata(args.concepts)

    selected_ids: Set[str] = set(selected_df["concept_id"].astype(str))
    known_ids: Set[str] = set(concepts_df["concept_id"].astype(str))

    missing_ids: List[str] = sorted(list(selected_ids - known_ids))
    print(f"Selected unique concept_ids: {len(selected_ids)}")
    ...
    if missing_ids:
        ...
        if args.show_combos:
            bad = (
                selected_df[selected_df["concept_id"].astype(str).isin(missing_ids)]["combo_id"]
                .astype(str)
                .dropna()
                .unique()
                .tolist()
