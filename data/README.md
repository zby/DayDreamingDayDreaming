...
**Generated by**: `group:task_definitions` assets (regenerate from actives) and curation scripts (for curated subsets)
...
- `1_raw/` and `2_tasks/` CSVs are overwritten in-place by their assets to reflect the latest selection logic, but are version-controlled in git. You can revert if needed. Note: `selected_combo_mappings.csv` is regenerated from active concepts when `group:task_definitions` is materialized; skip this asset if you want to keep a curated file intact.
