...
- `../2_tasks/selected_combo_mappings.csv` — optional curated combo selection (managed manually, no longer regenerated by assets)
...
**Contents**:
- `final_results.csv` - Aggregated experiment results for the cohort
...
- `2_tasks/selected_combo_mappings.csv` - optional curated selection (only updated by manual workflows)

**What's Not Tracked**:  
- `3_generation/` through `7_cross_experiment/` and `7_reporting/` - Large, regenerable, or operational data
...
- `1_raw/` CSVs and `cohorts/<cohort_id>/membership.csv` are overwritten in-place by their assets to reflect the latest selection logic, but are version-controlled in git. You can revert if needed. `2_tasks/selected_combo_mappings.csv` is no longer written by the pipeline—update it manually only if you maintain a curated subset.
