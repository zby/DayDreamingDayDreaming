...
def main():
    parser = argparse.ArgumentParser(
        description="Generate all prompts for given level and k"
    )
    parser.add_argument(
        "--level",
        default="paragraph",
        choices=["sentence", "paragraph", "article"],
    ...
    parser.add_argument(
        "--manifest",
        default="data/concepts/day_dreaming_concepts.json",
    ...
    if not combinations:
        print(
            f"Error: No {args.k}-combinations possible with {len(concept_db)} concepts"
        )
        return 1

    print(
        f"Generating prompts for {len(combinations)} combinations at '{args.level}' level"
    )
