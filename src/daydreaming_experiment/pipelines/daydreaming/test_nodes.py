...
def create_test_model_data():
    ...
    evaluation_templates = {"creativity_metrics": lambda: "Evaluate: {{ response }}"}
...
def create_test_concept_data():
    ...
    for i in range(1, 7):
        ...
        concept_descriptions_sentence[concept_id] = (
            lambda i=i: f"Sentence description for test concept {i}"
        )
        concept_descriptions_paragraph[concept_id] = (
            lambda i=i: f"Paragraph description for test concept {i}. This is more detailed content."
        )
        concept_descriptions_article[concept_id] = (
            lambda i=i: f"Article description for test concept {i}. This is the most detailed content available."
        )
...
class TestGenerateConceptCombinations:
    ...
    def test_generate_concept_combinations_k_max_2(self):
        ...
        concept_combinations_df, concept_combo_relationships_df = (
            generate_concept_combinations(concepts_list, parameters)
        )

        # Should have 3 choose 2 = 3 combinations
        assert len(concept_combinations_df) == 3
        assert (
            len(concept_combo_relationships_df) == 6
        )  # 3 combinations * 2 concepts each
        ...
        for combo_id in concept_combinations_df["combo_id"]:
            combo_concepts = concept_combo_relationships_df[
                concept_combo_relationships_df["combo_id"] == combo_id
            ]
    ...
    def test_generate_concept_combinations_k_max_3(self):
        ...
        assert (
            len(concept_combo_relationships_df) == 60
        )  # 20 combinations * 3 concepts each
    ...
    def test_generate_concept_combinations_description_format(self):
        ...
        concept_combinations_df, _ = generate_concept_combinations(
...
class TestCreateTaskList:
    ...
    def test_create_task_list_with_real_concepts(self):
        ...
        (
            concept_combinations_df,
            concept_combo_relationships_df,
            generation_tasks_df,
            evaluation_tasks_df,
            concept_contents,
        ) = create_task_list(
        ...
        for col in required_combinations_columns:
            assert (
                col in concept_combinations_df.columns
        ...
        for col in required_relationships_columns:
            assert (
                col in concept_combo_relationships_df.columns
            ), f"Missing required column: {col}"
        ...
        for concept_id, content in concept_contents.items():
            assert isinstance(
                concept_id, str
            ), f"Concept ID should be string: {concept_id}"
            assert isinstance(
                content, str
            ), f"Concept content should be string: {content}"
        ...
        for combo_id in concept_combinations_df["combo_id"]:
            ...
            assert (
                len(combo_concepts) == 2
            ), f"Expected 2 concepts for k_max=2, got {len(combo_concepts)}"
            ...
            assert len(set(concepts)) == len(
                concepts
            ), f"Duplicate concepts in combo: {concepts}"
    ...
    def test_create_task_list_with_k_max_3(self):
        ...
        for combo_id in concept_combinations_df["combo_id"]:
            ...
            assert (
                len(combo_concepts) == 3
            ), f"Expected 3 concepts for k_max=3, got {len(combo_concepts)}"
    ...
    def test_create_task_list_active_model_selection(self):
        ...
        for model in gen_models_used:
            assert (
                model in active_gen_models
            ), f"Inactive generation model used: {model}"
        ...
        for model in eval_models_used:
            assert (
                model in active_eval_models
            ), f"Inactive evaluation model used: {model}"
...
class TestGeneratePrompts:
    ...
    def test_generate_prompts_with_real_data(self):
        ...
        for generation_task_id in test_gen_tasks["generation_task_id"]:
            assert (
                generation_task_id in prompts
            ), f"Missing prompt for task {generation_task_id}"
            assert isinstance(
                prompts[generation_task_id], str
            ), f"Prompt for {generation_task_id} is not a string"
            assert (
                len(prompts[generation_task_id].strip()) > 0
            ), f"Empty prompt for {generation_task_id}"

        # Verify prompts contain concept content
        for generation_task_id, prompt in prompts.items():
            # Get combo for this task
            task_row = test_gen_tasks[
                test_gen_tasks["generation_task_id"] == generation_task_id
            ].iloc[0]
            ...
            for _, concept_row in combo_concepts.iterrows():
                ...
                assert (
                    concept_content in prompt
                ), f"Concept content for '{concept_id}' not found in prompt"
        ...
        assert len(unique_prompts) == len(
            prompts
        ), "Found duplicate prompts for different tasks"
