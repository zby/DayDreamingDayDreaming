        ...
        for attempt in range(self.max_retries + 1):  # +1 for initial attempt
            ...
            try:
                ...
                if self._is_retryable_error(e) and attempt < self.max_retries:
                    ...
                    logger.warning(f"API call failed (attempt {attempt + 1}/{self.max_retries + 1}): {e}. Retrying in {delay}s...")
                    time.sleep(delay)
                    continue
                else:
                    # Non-retryable error or max retries exceeded
                    if attempt == self.max_retries:
                        logger.error(f"API call failed after {self.max_retries + 1} attempts. Final error: {e}")
                    raise e
