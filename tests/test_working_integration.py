        ...
        if sentence_path.exists():
            with open(sentence_path, "r", encoding="utf-8") as f:
                concept_descriptions_sentence[concept_id] = (
                    lambda path=sentence_path: path.read_text(encoding="utf-8")
                )
        ...
        if paragraph_path.exists():
            with open(paragraph_path, "r", encoding="utf-8") as f:
                concept_descriptions_paragraph[concept_id] = (
                    lambda path=paragraph_path: path.read_text(encoding="utf-8")
                )
        ...
        if article_path.exists():
            with open(article_path, "r", encoding="utf-8") as f:
                concept_descriptions_article[concept_id] = (
                    lambda path=article_path: path.read_text(encoding="utf-8")
                )
...
CONCEPTS_DESCRIPTIONS_PATH = (
    Path(__file__).parent.parent / "data" / "01_raw" / "concepts" / "descriptions"
)

GEN_TEMPLATES_PATH = (
    Path(__file__).parent.parent / "data" / "01_raw" / "generation_templates"
)

EVAL_TEMPLATES_PATH = (
    Path(__file__).parent.parent / "data" / "01_raw" / "evaluation_templates"
)
...
class TestPipelineIntegration:
    ...
    def test_prompt_generation_integration(self):
        ...
        generation_templates_real = {
            "00_systematic_analytical": make_template_loader(
...
Please systematically explore how these concepts might be combined."""
        ...
        for generation_task_id, prompt in prompts.items():
            ...
            gen_task = sample_gen_tasks[
                sample_gen_tasks["generation_task_id"] == generation_task_id
            ].iloc[0]
...
class TestEvaluationTemplatesIntegration:
    ...
    def test_evaluation_templates_exist(self):
        ...
        assert (
            eval_templates_dir.exists()
        ), f"Evaluation templates directory must exist: {eval_templates_dir}"
        ...
        for template_file in expected_templates:
            ...
            ), f"Expected template file missing: {template_path}"
            ...
            with open(template_path, "r", encoding="utf-8") as f:
                ...
                ), f"Template missing REASONING section: {template_path}"
                assert (
                    "SCORE:" in content
                ), f"Template missing SCORE section: {template_path}"
...
class TestGenerationTemplatesIntegration:
    ...
    def test_generation_templates_exist(self):
        ...
        assert (
            gen_templates_dir.exists()
        ), f"Generation templates directory must exist: {gen_templates_dir}"
        ...
        for template_file in gen_templates_dir.glob("*.txt"):
            ...
            try:
                ...
                for concept in test_concepts:
                    assert (
                        concept["name"] in rendered
                    ), f"Concept name '{concept['name']}' not found in rendered template {template_file.name}"
...
class TestDataDirectoryStructure:
    ...
    def test_required_files_exist(self):
        ...
        concepts_metadata = pd.read_csv(CONCEPTS_METADATA_PATH)
        assert len(concepts_metadata) > 0, "Concept metadata must contain concepts"
...
class TestWorkflowIntegrationWithRealData:
    ...
    def test_pipeline_nodes_with_real_concepts_and_templates(self):
        ...
        assert (
            len(generation_tasks_df) > 0
        ), "Should generate tasks with real concept data"
        assert (
            len(concept_contents) >= 2
        ), "Need at least 2 concepts for combination testing"
        ...
        for generation_task_id, prompt in prompts.items():
            ...
            for _, concept_row in combo_concepts.iterrows():
                ...
                concept_name = concept_name_lookup[concept_id]
                assert (
                    concept_name in prompt
                ), f"Concept '{concept_name}' should appear in prompt"

            # Should contain template-specific content
            assert (
                "systematically" in prompt.lower()
            ), "Should contain template-specific language"
