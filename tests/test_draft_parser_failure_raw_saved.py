...
def test_draft_parser_failure_saves_raw_then_fails(tmp_path: Path):
    ...
    # Act + Assert: draft_response should raise Failure due to templates CSV missing 'generator' column
    # (parser is set to essay_block; this path validates config strictly)
    with pytest.raises(Failure):
        _ = draft_response_impl(ctx, draft_prompt="ignored")
