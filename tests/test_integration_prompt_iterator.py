...
class TestPromptIteratorIntegration:
    ...
    def concept_db(self):
        ...
        manifest_path = Path(__file__).parent.parent / "data" / "concepts" / "day_dreaming_concepts.json"
    ...
    def test_loads_real_concepts(self, concept_db):
        ...
        for concept in concept_db.get_concepts():
            assert concept.sentence is not None, f"Concept '{concept.name}' missing sentence"
            assert concept.paragraph is not None, f"Concept '{concept.name}' missing paragraph"
    ...
    def test_generates_prompts_with_real_data(self, concept_db, prompt_factory):
        ...
        for k, expected_prompts in test_cases:
            combinations = list(concept_db.get_combinations(k))[:1]  # Take just first combination
    ...
    def test_prompt_content_contains_real_concept_text(self, concept_db, prompt_factory):
    ...
    def test_end_to_end_pipeline_sample(self, concept_db, prompt_factory):
        ...
        for k in range(1, 4):  # k=1,2,3
            combinations = list(concept_db.get_combinations(k))[:2]  # First 2 combinations each
            iterator = PromptIterator(prompt_factory, combinations, "paragraph")
