...
class TestConcept:
    ...
    def test_get_description_non_strict_mode(self):
        ...
        concept_minimal = Concept(name="minimal", descriptions={"sentence": "Only sentence."})
        assert concept_minimal.get_description("paragraph", strict=False) == "Only sentence."
        
        # Should fallback to name when no content available
        concept_empty = Concept(name="empty_concept", descriptions={})
        assert concept_empty.get_description("paragraph", strict=False) == "empty_concept"
