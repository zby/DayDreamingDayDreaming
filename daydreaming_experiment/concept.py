...
class Concept:
    ...
    def get_description(self, level: str, strict: bool = True) -> str:
        ...
        if level not in self.DESCRIPTION_LEVELS:
            raise ValueError(f"Unknown description level '{level}'. Valid levels: {self.DESCRIPTION_LEVELS}")
        ...
        for i in range(requested_idx - 1, -1, -1):
            ...
            if fallback_level in self.descriptions and self.descriptions[fallback_level]:
